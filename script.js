...
function make_main_path()
{
	...
	for(i=0;;)
	{
		...
		if(proposed_location != false)
		{
			...
			{
				// the proposed location is taken, we need to get another chance
				proposed_direction = get_random_direction();
				console.log("The location was taken and so we re-elect the course to "+proposed_direction);
				var deadlock = check_if_deadlock(current_location);
				if(deadlock == true)
				{
					console.clear();
					console.log("("+current_location+") is a deadlock");
					deadlock_handler();
					break;
				}
				else
				{
					console.log("("+current_location+") is not a deadlock");
					continue;
				}
			}
		}
...
function make_branch(cell)
{
	...
	for(var i=0;;)
	{
		...
		if(proposed_location != false)
		{
			...
			{
				// it could either be a deadlock or simply be solved by changing course
				if(check_if_deadlock(current_location) == true)
				{
					console.log("We got a deadlock at "+current_location);
					deadlock_handler();
					break;
				}
				else
				{
					console.log("It is not a deadlock, a simple change of direction should solve the problem");
					console.log("We need to re-do the whole procedure !");
					continue;
				}
			}
		}
		else
		{
			...
			{
				console.log("It is not a deadlock, but the edge- we need to re-do it all");
				continue;
			}
		}
	}
